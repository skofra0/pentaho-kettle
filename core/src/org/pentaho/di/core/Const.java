...
public class Const {
     ...
     */
    public static final String KETTLE_COMPATIBILITY_MERGE_ROWS_USE_REFERENCE_STREAM_WHEN_IDENTICAL = "KETTLE_COMPATIBILITY_MERGE_ROWS_USE_REFERENCE_STREAM_WHEN_IDENTICAL";
     ...
     */
    public static final String VFS_USER_DIR_IS_ROOT = "vfs.sftp.org.apache.commons.vfs2.provider.sftp.SftpFileSystemConfigBuilder.USER_DIR_IS_ROOT";
    ...
    /*
     * OLD code: caused a lot of problems with very small and very large numbers. It's a miracle it worked at all. Go ahead, have a laugh... public static float round(double f, int places) { float temp = (float) (f * (Math.pow(10, places)));
     *
    ...
    /**
     * Convert a String into a date. The date format is <code>yyyy/MM/dd HH:mm:ss.SSS</code>. If the conversion fails, assign a default value.
     *
    ...
    public static String getUserHomeDirectory() {
        return NVL(System.getenv("KETTLE_HOME"), NVL(System.getProperty("KETTLE_HOME"), System.getProperty("user.home")));
    }
     ...
     *
     * @param uri the resource identifier for the documentation (eg. 0L0/0Y0/030/050/000)
     *
     ...
     *
     * <p>
     * <b>NOTE: this differs from String.split() in a way that the built-in method uses regular expressions and this one does not.</b>
     *
     ...
     *
     * The delimiter and enclosures are not regular expressions (regexes); rather they are literal strings that will be quoted so as not to be treated like regexes.
     *
     * This method expects that the data contains an even number of enclosure strings in the input; otherwise the results are undefined
     *
     * @param stringToSplit the String to split
     * @param delimiter the delimiter string
     * @param enclosure the enclosure string
     * @return an array of strings split on the delimiter (ignoring those in enclosures), or null if the string to split is null.
     */
    ...
    /**
     * <p>
     * Returns the pure filename of a filename with full path. E.g. if passed parameter is <code>/opt/tomcat/logs/catalina.out</code> this method returns <code>catalina.out</code>. The method works with the Environment variable <i>System.getProperty("file.separator")</i>, so on linux/Unix it will check for the last occurrence of a frontslash, on windows for the last occurrence of a backslash.
     * </p>
     *
     * <p>
    ...
    public static String[] getTransformationFilterNames() {
        if (STRING_TRANS_FILTER_NAMES == null) {
            STRING_TRANS_FILTER_NAMES = new String[] {BaseMessages.getString(PKG, "Const.FileFilter.Transformations"), BaseMessages.getString(PKG, "Const.FileFilter.XML"), BaseMessages.getString(PKG, "Const.FileFilter.All")};
        }
    ...
    public static String[] getJobFilterNames() {
        if (STRING_JOB_FILTER_NAMES == null) {
            STRING_JOB_FILTER_NAMES = new String[] {BaseMessages.getString(PKG, "Const.FileFilter.Jobs"), BaseMessages.getString(PKG, "Const.FileFilter.XML"), BaseMessages.getString(PKG, "Const.FileFilter.All")};
        }
