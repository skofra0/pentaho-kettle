	...
	public String getFieldDefinition(ValueMetaInterface v, String tk, String pk, boolean use_autoinc, boolean add_fieldname, boolean add_cr) {
		...
		case ValueMetaInterface.TYPE_BOOLEAN:
			if (supportsBooleanDataType()) {
				retval += "BOOLEAN COMMENT \"LOOKUP\"";
			} else {
				retval += "CHAR(1) COMMENT \"LOOKUP\"";
			}
			break;
				...
				if (precision == 0) {
					if (length > 9) {
						if (length < 16) {
							...
							retval += "BIGINT(" + length + ") COMMENT \"LOOKUP\"";
						} else if (length < 19) {
							// can hold signed values between
							// -9223372036854775808 and 9223372036854775807
							// 18 significant digits
						...
						if (length > 0) {
							retval += "INT(" + length + ") COMMENT \"LOOKUP\"";
						} else {
							retval += "INT  COMMENT \"LOOKUP\"";
						}
					}
				}
		...
		case ValueMetaInterface.TYPE_STRING:
			if (length > 0) {
				if (length == 1)
					...
					retval += "VARCHAR(" + length + ") COMMENT \"LOOKUP\"";
				else if (length < 256)
