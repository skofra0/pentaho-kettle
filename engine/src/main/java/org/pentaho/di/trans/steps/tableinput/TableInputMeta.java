...
public class TableInputMeta extends BaseStepMeta implements StepMetaInterface {
     ...
     */
    public void setExecuteEachInputRowAsPreparedStatment(boolean oncePerRow) {
        this.executeEachInputRowAsPreparedStatment = oncePerRow;
    }
    ...
    public String getXML() {
        ...
        retval.append("    " + XMLHandler.addTagValue("execute_each_row", executeEachInputRowAsPreparedStatment));
    ...
    public void saveRep(Repository rep, IMetaStore metaStore, ObjectId id_transformation, ObjectId id_step) throws KettleException {
        try {
            ...
            rep.saveStepAttribute(id_transformation, id_step, "execute_each_row", executeEachInputRowAsPreparedStatment);
    ...
    public void setDefaultVariables(VariableSpace space) {
        for (int i = 0; i < getFieldName().length; i++) {
            if (!Utils.isEmpty(getFieldName()[i])) {
