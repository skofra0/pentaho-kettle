...
public class TableInput extends BaseStep implements StepInterface {
    ...
    public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
        if (first) { // we just got started
            ...
            if (data.infoStream.getStepMeta() != null) {
                if (meta.isExecuteEachInputRowAsPreparedStatment()) {
                    ...
                    // data.rowSet = findInputRowSet(data.infoStream.getStepname());
                    // if (data.rowSet == null) {
                    // throw new KettleException("Unable to find rowset to read from");
                    // }
                    // setAllRowVariables(getRowFrom(data.rowSet));
                    //
                    setAllRowVariables(parameters);
            ...
            if (meta.isExecuteEachInputRowAsPreparedStatment() && (parameters == null || parametersMeta.size() == 0)) {
        ...
        if (data.thisrow == null) { // Finished reading?
            ...
            boolean done = false;
            if (meta.isExecuteEachInputRowAsPreparedStatment()) { // Try to get another row from the input stream
    ...
    private void setValue(RowSet rowSet, Object[] row, int i, boolean usedefault) throws KettleException { // SKOFRA
        ...
        if (usedefault) {
            ...
            int index = rowSet.getRowMeta().indexOfValue(meta.getFieldName()[i]);
            if (index < 0) {
                throw new KettleException("Unable to find field [" + meta.getFieldName()[i] + "] in input row");
            }
            ValueMetaInterface valueMeta = rowSet.getRowMeta().getValueMeta(index);
