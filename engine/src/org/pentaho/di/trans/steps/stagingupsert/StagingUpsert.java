...
public class StagingUpsert extends BaseStep implements StepInterface {
    ...
    private StagingUpsertMeta meta;
    private StagingUpsertData data;

    public StagingUpsert(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta, Trans trans) {
    ...
    private synchronized void lookupValues(RowMetaInterface rowMeta, Object[] row) throws KettleException {
        ...
        if (log.isDebug()) {
            logDebug(BaseMessages.getString(PKG, "StagingUpsert.Log.ValuesSetForLookup") + data.lookupParameterRowMeta.getString(lookupRow));
        }
        ...
        if (add == null) {
             ...
             */
            if (log.isRowLevel()) {
                logRowlevel(BaseMessages.getString(PKG, "StagingUpsert.InsertRow") + rowMeta.getString(row));
            }
            ...
            if (log.isRowLevel()) {
                logRowlevel(BaseMessages.getString(PKG, "StagingUpsert.Log.FoundRowForUpdate") + rowMeta.getString(row));
            }
            ...
            if (update) {
                ...
                if (log.isRowLevel()) {
                    logRowlevel(BaseMessages.getString(PKG, "StagingUpsert.Log.UpdateRow") + data.lookupParameterRowMeta.getString(lookupRow));
                }
    ...
    public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
        meta = (StagingUpsertMeta) smi;
        data = (StagingUpsertData) sdi;
        ...
        if (first) {
            ...
            // lookup the values!
            if (log.isDebug()) {
                logDebug(BaseMessages.getString(PKG, "StagingUpsert.Log.CheckingRow") + getInputRowMeta().getString(r));
            }
            ...
            for (int i = 0; i < meta.getKeyStream().length; i++) {
                ...
                if (keynr < 0 && // couldn't find field!
                    ...
                    throw new KettleStepException(BaseMessages.getString(PKG, "StagingUpsert.Exception.FieldRequired", meta.getKeyStream()[i]));
                }
                keynrs.add(keynr);
                ...
                if (keynr2 < 0 && // couldn't find field!
                    ...
                    throw new KettleStepException(BaseMessages.getString(PKG, "StagingUpsert.Exception.FieldRequired", meta.getKeyStream2()[i]));
                }
                keynrs2.add(keynr2);

                if (log.isDebug()) {
                    logDebug(BaseMessages.getString(PKG, "StagingUpsert.Log.FieldHasDataNumbers", meta.getKeyStream()[i]) + "" + keynrs.get(keynrs.size() - 1));
                }
            }
            ...
            for (int i = 0; i < meta.getUpdateLookup().length; i++) {
                ...
                if (data.valuenrs[i] < 0) {
                    ...
                    throw new KettleStepException(BaseMessages.getString(PKG, "StagingUpsert.Exception.FieldRequired", meta.getUpdateStream()[i]));
                }
                if (log.isDebug()) {
                    logDebug(BaseMessages.getString(PKG, "StagingUpsert.Log.FieldHasDataNumbers", meta.getUpdateStream()[i]) + data.valuenrs[i]);
                }
            }
        ...
        try {
            ...
            if (getStepMeta().isDoingErrorHandling()) {
                ...
                throw new KettleStepException(BaseMessages.getString(PKG, "StagingUpsert.Log.ErrorInStep"), e);
            }
        ...
        if (super.init(smi, sdi)) {
            try {
                if (meta.getDatabaseMeta() == null) {
                    logError(BaseMessages.getString(PKG, "StagingUpsert.Init.ConnectionMissing", getStepname()));
                    return false;
                }
                ...
                if (autoCreateTables.startsWith("Y") && !data.db.checkTableExists(environmentSubstitute(schemaTable)) ) {
                ...
                logError(BaseMessages.getString(PKG, "StagingUpsert.Log.ErrorOccurredDuringStepInitialize") + ke.getMessage());
            }
        }
        return false;
    }
    ...
    public void dispose(StepMetaInterface smi, StepDataInterface sdi) {
        ...
        if (data.db != null) {
            try {
                ...
                logError(BaseMessages.getString(PKG, "StagingUpsert.Log.UnableToCommitConnection") + e.toString());
                setErrors(1);
            } finally {
                data.db.disconnect();
            }
