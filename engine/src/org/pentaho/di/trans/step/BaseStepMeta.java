...
/**
 * This class is responsible for implementing common functionality regarding step meta, such as
 * logging. All Kettle steps have an extension of this where private fields have been added with
 * public accessors.
 * <p>
 * For example, the "Text File Output" step's TextFileOutputMeta class extends BaseStepMeta by
 * adding fields for the output file name, compression, file format, etc...
 * <p>
 *
...
public class BaseStepMeta implements Cloneable, StepAttributesInterface {
    ...
    public Object clone() {
        try {
            ...
            // - we cannot clone all existing streams, as the duplicate should be treated
            // independently
            // - we cannot clear ioMeta's stream list, because other classes do not expect the list
            // to be empty
     ...
     * @param nextStep the next step that is targeted
     * @param space the space The variable space to use to replace variables
     * @param repository the repository to use to load Kettle metadata objects impacting the output
     *        fields
     * @param metaStore the MetaStore to use to load additional external data or metadata impacting
     *        the output fields
     * @throws KettleStepException the kettle step exception
     */
    ...
    /**
     * Standard method to return one or more SQLStatement objects that the step needs in order to
     * work correctly. This can mean "create table", "create index" statements but also "alter table
    ...
    /**
     * Standard method to return an SQLStatement object with SQL statements that the step needs in
     * order to work correctly. This can mean "create table", "create index" statements but also
     * "alter table ... add/drop/modify" statements.
     *
     * @return The SQL Statements for this step. If nothing has to be done, the
    ...
    /**
     * Default a step doesn't use any arguments. Implement this to notify the GUI that a window has
     * to be displayed BEFORE launching a transformation.
     *
    ...
    /**
     * The natural way of data flow in a transformation is source-to-target. However, this makes
     * mapping to target tables difficult to do. To help out here, we supply information to the
     * transformation meta-data model about which fields are required for a step. This allows us to
     * automate certain tasks like the mapping to pre-defined tables. The Table Output step in this
     * case will output the fields in the target table using this method.
     *
     * This default implementation returns an empty row meaning that no fields are required for this
     * step to operate.
     *
     ...
     *
     * @return an array of database connections meta-data. Return an empty array if no connections
     *         are used.
     */
    ...
    /**
     * @return true if this step supports error "reporting" on rows: the ability to send rows to a
     *         certain target step.
     */
    ...
    public List<ResourceReference> getResourceDependencies(TransMeta transMeta, StepMeta stepInfo) {
        List<ResourceReference> references = new ArrayList<ResourceReference>(5); // Lower the
                                                                                  // initial
                                                                                  // capacity -
                                                                                  // unusual
                                                                                  // to have more
    ...
    /**
     * This returns the expected name for the dialog that edits a job entry. The expected name is in
     * the org.pentaho.di.ui tree and has a class name that is the name of the job entry with
     * 'Dialog' added to the end.
     *
     * e.g. if the job entry is org.pentaho.di.job.entries.zipfile.JobEntryZipFile the dialog would
     * be org.pentaho.di.ui.job.entries.zipfile.JobEntryZipFileDialog
     *
     * If the dialog class for a job entry does not match this pattern it should override this
     * method and return the appropriate class name
     *
    ...
    /**
     * Returns the Input/Output metadata for this step. By default, each step produces and accepts
     * optional input.
     */
     ...
     *         possible actions like "New target step"
     */
    ...
    /**
     * @return Optional interface that allows an external program to inject step metadata in a
     *         standardized fasion. This method will return null if the interface is not available
     *         for this step.
     */
    ...
    /**
     * @return The step metadata itself, not the metadata description. For lists it will have 0
     *         entries in case there are no entries.
     ...
     * (non-Javadoc)
     *
     * @see org.pentaho.di.trans.step.StepAttributesInterface#findParent(java.util.List,
     * java.lang.String)
     */
    ...
    /**
     * @return A description of the active referenced object in a transformation. Null if nothing
     *         special needs to be done or if the active metadata isn't different from design.
     */
