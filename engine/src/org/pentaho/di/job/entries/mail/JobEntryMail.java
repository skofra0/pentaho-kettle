...
public class JobEntryMail extends JobEntryBase implements Cloneable, JobEntryInterface {
    ...
    public String getXML() {
        ...
        retval.append("      ").append(XMLHandler.addTagValue("configFile", configFile)); // SKOFRA
    ...
    public void loadRep(Repository rep, IMetaStore metaStore, ObjectId id_jobentry, List<DatabaseMeta> databases, List<SlaveServer> slaveServers) throws KettleException {
        try {
            ...
            //
            configFile = rep.getJobEntryAttributeString(id_jobentry, "configFile"); // SKOFRA
    ...
    public void saveRep(Repository rep, IMetaStore metaStore, ObjectId id_job) throws KettleException {
        try {
            rep.saveJobEntryAttribute(id_job, getObjectId(), "configFile", configFile); // SKOFRA
    ...
    public void setConfigFile(String configFile) { // SKOFRA
        this.configFile = configFile;
    }

    public String getConfigFile() { // SKOFRA
    ...
    public Result execute(Result result, int nr) {
        ...
        try {
            if (usingConfig) {
                String path = environmentSubstitute(getConfigFile());
                InputStream inStream = Files.newInputStream(Paths.get(path));
                props.load(inStream);
                if (!Const.toStringNullToEmpty(props.get("mail.smtps.host")).isEmpty()) {
                    protocol = "smtps";
                }
                dataServer = Const.toStringNullToEmpty(props.get("mail." + protocol + ".host"));
                dataAuthenticationUser = Const.toStringNullToEmpty(props.get("mail." + protocol + ".user"));
                dataAuthenticationPassword = Const.toStringNullToEmpty(props.get("mail." + protocol + ".password"));
                dataUsingAuthentication = Const.toStringNullToEmpty(props.get("mail." + protocol + ".auth")).equalsIgnoreCase("true");
                dataPort = Const.toStringNullToEmpty(props.get("mail." + protocol + ".port"));
            } else {
                ...
                props.put("mail." + protocol + ".host", environmentSubstitute(dataServer));
                if (!Const.isEmpty(dataPort)) {
                    props.put("mail." + protocol + ".port", environmentSubstitute(dataPort));
                }
                if (dataUsingAuthentication) {
            ...
            try {
                ...
                String authPass = getPassword(dataAuthenticationPassword); // SKOFRA
