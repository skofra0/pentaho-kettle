...
package plugin.nexus.swt.repo.controller;
...
import plugin.nexus.swt.repo.IConnectedRepositoryInstance;
import plugin.nexus.swt.repo.model.RepositoryModel;
import plugin.nexus.swt.repo.timeout.RepositorySessionTimeoutHandler;
...
public class RepositoryConnectController implements IConnectedRepositoryInstance {
    ...
    public static final String HELP_URL = Const.getDocUrl(BaseMessages.getString(PKG, "RepositoryDialog.Dialog.Help"));
    ...
    public boolean help() {
        spoonSupplier.get().getShell().getDisplay().asyncExec(() -> HelpUtils.openHelpDialog(this.parentShell, BaseMessages.getString(PKG, "RepositoryDialog.Dialog.Tile"), HELP_URL, BaseMessages.getString(PKG, "RepositoryDialog.Dialog.Header")));
        return true;
    }
    ...
    private Repository loadRepositoryObject(String id) throws KettleException {
        Repository repository = pluginRegistry.loadClass(RepositoryPluginType.class, id, Repository.class);
