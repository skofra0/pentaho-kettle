...
package plugin.nexus.swt.fileopensave.providers.repository;
...
import plugin.nexus.swt.fileopensave.api.providers.BaseFileProvider;
import plugin.nexus.swt.fileopensave.api.providers.exception.FileException;
import plugin.nexus.swt.fileopensave.api.providers.exception.FileExistsException;
import plugin.nexus.swt.fileopensave.api.providers.exception.InvalidFileOperationException;
import plugin.nexus.swt.fileopensave.api.providers.exception.InvalidFileTypeException;
import plugin.nexus.swt.fileopensave.controllers.RepositoryBrowserController;
import plugin.nexus.swt.fileopensave.providers.repository.model.RepositoryDirectory;
import plugin.nexus.swt.fileopensave.providers.repository.model.RepositoryFile;
import plugin.nexus.swt.fileopensave.providers.repository.model.RepositoryTree;
import plugin.nexus.swt.fileopensave.util.Util;
...
public class RepositoryFileProvider extends BaseFileProvider<RepositoryFile> {
    ...
    private boolean isTransOpened(String id, String path, String name) {
        ...
        for (TransMeta t : openedTransFiles) {
            if (t.getObjectId() != null && id.equals(t.getObjectId().getId()) || (path.equals(t.getRepositoryDirectory().getPath()) && name.equals(t.getName()))) {
                return true;
            }
        }
        return false;
    ...
    private boolean isJobOpened(String id, String path, String name) {
        ...
        for (JobMeta j : openedJobFiles) {
            if (j.getObjectId() != null && id.equals(j.getObjectId().getId()) || (path.equals(j.getRepositoryDirectory().getPath()) && name.equals(j.getName()))) {
                return true;
            }
        }
        return false;
    ...
    @Override
    public boolean isSame(plugin.nexus.swt.fileopensave.api.providers.File file1, plugin.nexus.swt.fileopensave.api.providers.File file2) {
