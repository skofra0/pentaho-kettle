...
public class LibFormulaEditor extends Dialog implements KeyListener {
	...
	public void keyPressed(KeyEvent e) {
		...
		if (ctrl && e.character==' ') {
	      ...
	      // SKOFRA FIX proposals.add(new CompletionProposal("["+fieldName+"] (keyword)", fieldName.substring(before.length())+"]", fieldName.length()-before.length()+1));
						proposals.add(new CompletionProposal("["+fieldName+"] (keyword)", fieldName.substring(before.length()-1)+"]", fieldName.length()-before.length()+1));
					}
				}
